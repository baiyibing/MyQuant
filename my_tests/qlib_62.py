...
if __name__ == '__main__':
    ...
    print("回测结果摘要:")
    print(f"年化收益率: {report_normal_df.get('annualized_return', 'N/A')}")
    print(f"夏普比率: {report_normal_df.get('sharpe_ratio', 'N/A')}")
    print(f"最大回撤: {report_normal_df.get('max_drawdown', 'N/A')}")
    print(f"信息比率: {report_normal_df.get('information_ratio', 'N/A')}")

    # 可视化分析
    print("生成可视化报告...")
    analysis_position.report_graph(report_normal_df)  # 累计收益曲线
    ...
    analysis_position.score_ic_graph(pred_label, freq="day")  # IC分析 ERROR - qlib.workflow - [utils.py:41] - An exception has been raised[KeyError: 'label'].
