...
if __name__ == '__main__':
    ...
    # 使用工作流记录实验
    with R.start(experiment_name="feature_importance_analysis"):
        # 训练模型
        model.fit(dataset)
        ...
        # 可选：保存特征重要性结果
        # R.save_objects(feature_importance=importance_df)
    ...
    plt.title("Top 15 Feature Importance (Qlib LGBModel)")
    plt.gca().invert_yaxis()
    plt.tight_layout()
    plt.savefig("feature_importance_plot.png", dpi=300, bbox_inches="tight")
    plt.show()

    print("可视化图表已保存为 feature_importance_plot.png")
