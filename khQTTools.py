...
def download_and_store_data(local_data_path, stock_files, field_list, period_type, start_date, end_date, dividend_type='none', time_range='all', progress_callback=None, log_callback=None):
    ...
    try:
        ...
        # 将字符串转换为date对象
        date1_obj = datetime.strptime(start_date, "%Y%m%d").date()
        date2_obj = datetime.strptime(end_date, "%Y%m%d").date()
        # 计算差值
        delta = date2_obj - date1_obj
        days = abs(delta.days)

        if period_type == '1d' and days <= 30 :
            xtdata.download_history_data2(
                stock_list=stocks,
                period=period_type,
                start_time=start_date,
                end_time=end_date,
                callback=on_progress  # 可选进度监控
            )
            ...
            for stock, df in data.items():
                if not df.empty:
                    ...
                    logging.info(f"保存parquet文件 - 路径: {dividend_type}/{stock}")
                    merge_and_deduplicate(df, stock, dividend_type)
                    logging.info(f"parquet文件保存成功: {dividend_type}/{stock}")
                ...
                # df['symbol'] = stock_code  # 添加股票代码列
                # combined_dfs.append(df)

            # result_df = pd.concat(combined_dfs)  # 合并为单一DataFrame
...
def on_progress(data):
    """下载进度回调函数"""
    logging.info(f"进度: {data['finished']}/{data['total']}, 当前股票: {data['stockcode']}")

def merge_and_deduplicate(new_data: pd.DataFrame, symbol: str, dividend_type: str):
    """合并新数据并基于 (symbol, date) 去重"""
    DATA_DIR = "a_kline"  # 根目录
    PARQUET_OPTIONS = {"compression": "zstd"}  # ZSTD 压缩
    partition_dir = os.path.join(DATA_DIR, f"dividend_type={dividend_type}", f"symbol={symbol}")
    os.makedirs(partition_dir, exist_ok=True)
    history_file = os.path.join(partition_dir, f"data.parquet")
    ...
    combined_df.to_parquet(history_file, **PARQUET_OPTIONS)
