...
from khQTTools import download_and_store_data,get_and_save_stock_list, supplement_history_data, merge_and_deduplicate
...
class StockDataCleaner:
    ...
    def save_cleaned_data(self, file_path):
        ...
        # 文件名格式：{stock}_{period_type}_{start_date}_{end_date}_{time_range_filename}_{dividend_type}.csv
        file_name  = os.path.basename(file_path)
        base_name = os.path.splitext(file_name)[0]
        segments = base_name.split('_')
        stock = segments[0]
        dividend_type = segments[5]
        self.df["date"] = self.df["date"].astype("str")
        merge_and_deduplicate(self.df,stock,dividend_type)
