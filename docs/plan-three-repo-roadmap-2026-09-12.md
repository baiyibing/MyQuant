...
- 日期：2026-09-12（v1.3 回写 2026-09-13；§0.2 补 M4/M3-A/M2/M3-B/C/D + 宿主真跑；**M3D-D 同日收官**）
...
| M3-D 行业/市值中性化 | **收官（实验开关保留，线上默认关）** | 实现 #25 任务书 → #26（A `ranking_neutralize` / B `verify_float_cap` / C `export --neutralize`，CI 绿）；行业源此前 100% 复核。**M3D-D 宿主两窗四列**（raw/industry/size/both；命中率口径=次日命中）：三月窗 15 日对齐——与 raw 日重叠 **11.3 / 9.3 / 10.0%**，命中 **43.3 / 45.3 / 43.3%**（raw 44.0）；长窗 131 日——重叠 **85.6 / 91.7 / 84.0%**（均 **>80%**），命中 **45.6 / 46.0 / 45.8%**（raw 46.5）。按预锁：①长窗命中「不改名单」合法终点；②三月大换名单但命中原地踏步；③两窗同向不通过（命中差 <1.5pp 噪声级）→ **无采纳证据、也无害证据**；④脚注：三月 industry 后 Top1 行业占比反升 23.3→38%（去均值≠分散）；⑤日换手 88~95% 与中性化无关。结论：**保留实验开关、默认关闭、不改 topk**。过程教训：备料脚本须 `if __name__ == "__main__"` guard（Windows spawn 否则 worker 递归） |
...
| M3 | 模型迭代（只动排序，不动成交） | **中期轨道收官**。M3-A~C 已合并宿主验收；M3-D #26 代码合入 + M3D-D 两窗四列：**不采纳中性化（默认关）**；OOS 已否决单窗换参 |
| M4 | run manifest 约定 | **已做**。#9/#14：`myquant.run-manifest/1`；训练/导出接线；启动时 git 溯源 |
