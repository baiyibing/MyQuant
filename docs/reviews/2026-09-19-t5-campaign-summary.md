...
以下是研究顺序建议，不是事实包已经验证的收益结论。

- **优先预注册组合约束研究。**围绕头部换入方向、持仓暴露和换手，明确约束规则、对照、冻结指标及停止条件。MAXRET 的头部约束提供待验证线索；WRD1 应先保持头部、底部与宇宙基准的口径分离，不照搬 anti-rank 过滤。
- **配套分钟成交验证。**在独立研究方案中检查分钟级可成交性、成交价格、未成交处理、交易成本与换手影响，再判断信号变化能否进入实际组合。事实包没有这部分结果，本文不填写成交率、滑点或收益提升数字。
- **停止继续堆叠特征刀。**不凭 A 门通过继续加列、交互或扫描窗口、方向、seed；保留现有终态，不用伪实验或事后更换指标复活失败候选。研究提案与线上冻结分开管理。
