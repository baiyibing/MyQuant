...
Expected differences (explain and preserve, do not strip from the products):

- the `research_acceleration: TRACK_B_PLAN_HASH_CACHE_PENDING_REVIEW` stamp in rules `metadata.json` and portfolio `manifest.json` `metadata` (and in snapshot metadata when the freeze metadata carries it);
- the output-path URI string `inputs.plans.uri` (rules `metadata.json`, `rule-manifest.json` `plans.uri`, snapshot/portfolio metadata) when a new output directory is used;
- `run_id` in the portfolio manifest, if a different run id is used;
- consequently the bytes/SHA256 of rules `metadata.json`, `rule-manifest.json`, `snapshot.json` and portfolio `manifest.json`, and the portfolio manifest `snapshot` source seal (`uri`, `raw_sha256`, `content_sha256` of the whole snapshot). The baseline snapshot id `8a2854d8...` is therefore not expected to be reproduced byte-for-byte; the snapshot must instead match after removing only the stamp and plans URI.

Required equal: `plans.json` bytes and content hash (`e56994d8...`); the declared plans `raw_sha256`/`content_sha256` seals (they hash only the plans bytes/content, never the URI, so a drift there is a real product change, not a path artifact); snapshot `scores`/`initial_state`/`plans` sections; `reference_states` in `rule-manifest.json` and portfolio `manifest.json` (the on-disk packs do not write `final_states`; the last per-arm `reference_states[].after` stands in for it); `intents.csv`, `constraints.csv`, and `pref_check.json` bytes; `intent_hash` / `arm_intent_hashes`; `contract_hash`. Report all differences and the gate result before merge. Operator kit (PowerShell commands, `patch_freeze_metadata.py`, `ab_compare.py`): [ab-2026-09-25/](ab-2026-09-25/README.md). Scratch prototypes remain off-repo and are not part of this change.
