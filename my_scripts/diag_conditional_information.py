# -*- coding: utf-8 -*-
"""T5-DSTR1 A-gate: DSTR partial RankIC controlling frozen 8a061ea4 score."""
...
def _quarter_stability(daily, col="partial_rankic", full_window_mean=None):
    work = daily.copy()
    ...
    full = full_window_mean if full_window_mean is not None else (float(np.nanmean(means)) if means else float("nan"))
...
def decide_verdict(windows, data_ok: bool):
    ...
    opp_sign = bool((m25 > 0 and m26 < 0) or (m25 < 0 and m26 > 0))
    flip = bool(opp_sign or q.get("majority_negative"))
    s1 = bool(same_pos and ci_lo > 0 and q.get("passes"))
    if not s0:
        verdict = "DSTR_DATA_INVALID"
    elif flip:
        verdict = "DSTR_CONDITIONAL_FLIP"
    elif not s1:
        verdict = "DSTR_CONDITIONAL_NO_EDGE"
    else:
        verdict = "A_GATE_PASS"
    return {
        "verdict": verdict,
...
def _qc_sidecar_formula(sc, feature: str) -> list[str]:
    errors = []
    finite = sc[feature].notna() & np.isfinite(sc[feature])
    if int((finite & ((sc[feature] <= 0) | (sc[feature] > 1))).sum()):
        errors.append("DOWNSTREAK5_RANK out of (0, 1]")
    ds = pd.to_numeric(sc["downstreak5"], errors="coerce")
    ds_ok = ds.notna() & np.isfinite(ds)
    if int((ds_ok & ((ds < 0) | (ds > MAX_DAYS) | (np.mod(ds, 1.0) != 0))).sum()):
        errors.append("downstreak5 outside integer {0,1,2,3,4,5}")
    ranked_nan = int((ds_ok & sc[feature].isna()).sum())
    if ranked_nan:
        errors.append(f"finite downstreak5 missing rank on {ranked_nan} rows")
    mask = ds_ok & (sc["neg_bitmap"].astype(str).str.len() == MAX_DAYS)
    if int(mask.sum()):
        bm = sc.loc[mask, "neg_bitmap"].astype(str)
        stored = ds.loc[mask].astype(int).to_numpy()
        pref = np.zeros(int(mask.sum()), dtype=int)
        still = np.ones(int(mask.sum()), dtype=bool)
        for i in range(MAX_DAYS):
            is_neg = still & (bm.str[i].to_numpy() == "1")
            pref = pref + is_neg.astype(int)
            still = still & is_neg
        n_bad = int((pref != stored).sum())
        if n_bad:
            errors.append(f"neg_bitmap prefix != downstreak5 on {n_bad} rows")
        n_down = (
        ...
        if int(interleaved.sum()) and int((interleaved & (stored == n_down)).sum()):
            errors.append("interleaved rows have streak==down-day-count (CNTN5)")
        starts_zero = (bm.str[0] == "0").to_numpy()
        if int((starts_zero & (stored != 0)).sum()):
            errors.append("empty product treated as 1")
    return errors
...
def main(argv=None):
    ...
    if int(args.block_days) != BLOCK_DAYS or int(args.bootstrap_reps) != BOOTSTRAP_REPS or int(args.seed) != SEED:
        raise ValueError("bootstrap is frozen at 5 / 10000 / 20260918")
    ...
    if str(manifest.get("feature_name")) != FEATURE:
        raise RuntimeError("manifest feature_name is not DOWNSTREAK5_RANK")
    if int(manifest.get("multiply_before_rank", 0)) != 1:
        raise RuntimeError("manifest multiply_before_rank must be +1 (rank downstreak5 itself, no sign flip)")
    if int(manifest.get("max_market_days", 0)) != MAX_DAYS:
        raise RuntimeError("manifest max_market_days must be 5")
    if int(manifest.get("warmup_preceding_market_days", 0)) != MAX_DAYS:
        raise RuntimeError("manifest warmup_preceding_market_days must be 5")
    if not manifest.get("no_window_extension") or not manifest.get("no_close_imputation"):
        raise RuntimeError("manifest must freeze no-window-extension / no-close-imputation")
    if manifest.get("streak_side") != "down":
        raise RuntimeError("manifest streak_side must be down")
    if not manifest.get("exclude_zero_return"):
        raise RuntimeError("manifest must freeze exclude_zero_return")
    if not manifest.get("no_sign_flip"):
        raise RuntimeError("manifest must freeze no_sign_flip")
    ...
    formula_errors = _qc_sidecar_formula(sc, args.feature)
    data_ok = not bool(formula_errors)
    data_errors = list(formula_errors)
    ...
    for name, (start, end) in windows.items():
        ...
        miss_counts = (
            merged.loc[merged[args.feature].isna() | ~np.isfinite(merged[args.feature]), "missing_reason"]
            .fillna("unknown")
            .value_counts()
            .to_dict()
        )
        ...
        if daily.empty:
            ...
            qstab = (
                _quarter_stability(daily, full_window_mean=stats["mean"]) if name == "2026_oos" else None
            )
        summaries[name] = {
            ...
            "missing_reason_counts": miss_counts,
        }
        if len(daily):
    ...
    payload = {
        ...
        "no_rerank_on_a_subsample": True,
        "residualization_a_only": True,
    }
