...
def main() -> int:
    ...
    print(f"[run] pred={rec.id} rows={len(score)} st=ON age=ON return_threshold=OFF", flush=True)
    ...
    print(f"[run] nav_end={nav_end:.2f} excess_ann={exc['annualized_return']:.6f} ir={exc['information_ratio']:.6f}", flush=True)
    ...
    write_analysis_bundle(
        ...
        use_qlib_close=True,
        extra_summary={
            "source": "replay_same_pred_buy_st_age",
            "pred_recorder_id": rec.id,
            "st_filter": True,
            "age_filter": True,
        },
    )
    base_nav = float(pd.read_csv(BASE / "nav_daily.csv").iloc[-1]["account"])
    print(f"[cmp] no_filter {base_nav:.2f}  buy_st_age {nav_end:.2f}  delta {nav_end - base_nav:.2f}", flush=True)
    print(json.dumps({"pred": rec.id, "no_filter_nav": base_nav, "buy_st_age_nav": nav_end}, ensure_ascii=False), flush=True)
    return 0
