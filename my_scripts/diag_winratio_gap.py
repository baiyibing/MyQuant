# -*- coding: utf-8 -*-
"""T5-WRD1 A-gate: WR-gap partial RankIC controlling score + sidecar q."""
...
def build_parser():
    ...
    p.add_argument("--feature", default="BROKER_WR_GAP")
    p.add_argument("--control-q-column", default="q")
    p.add_argument("--control-cyq-column", default="exact_winner_ratio")
    p.add_argument("--consume-stored-q-g-only", action="store_true")
    p.add_argument("--recompute-construction-for-check-only", action="store_true")
    p.add_argument("--require-columns", default="instrument,datetime,broker_winratio,exact_winner_ratio,q,v,BROKER_WR_GAP,ols_n,degenerate_reason")
    p.add_argument("--verify-key-digest", action="store_true")
    p.add_argument("--verify-same-construction-sample", action="store_true")
    p.add_argument("--verify-residual-orthogonality", action="store_true")
    ...
    p.add_argument("--coverage-denominator", default="handler-finite-score")
...
def _partial_day(day, feature, qcol, min_n):
    if len(day) < min_n:
    ...
    g = day[feature]
    q = day[qcol]
    if s.nunique() < 2 or y.nunique() < 2 or g.nunique() < 2 or q.nunique() < 2:
        return float("nan"), "zero_variance"
    try:
        rg = _ols_resid(g.to_numpy(), s.to_numpy(), q.to_numpy())
        ry = _ols_resid(y.to_numpy(), s.to_numpy(), q.to_numpy())
    except Exception:
        return float("nan"), "ols_fail"
    if not (np.isfinite(rg).all() and np.isfinite(ry).all()):
        return float("nan"), "resid_nonfinite"
    if np.nanstd(rg) <= 0 or np.nanstd(ry) <= 0:
        return float("nan"), "resid_zero_var"
    corr = float(np.corrcoef(rg, ry)[0, 1])
...
def _series_stats(series):
    ...
    return {"n_days": int(len(v)), "mean": mean, "median": float(v.median()) if len(v) else float("nan"), "std": std, "icir": mean / std if np.isfinite(std) and std > 0 else float("nan")}
...
def _quarter_stability(daily, col="partial_rankic"):
    ...
    return {"quarters": rows, "majority_negative": majority_negative, "single_quarter_driven": single, "passes": (not majority_negative) and (not single)}


def decide_verdict(windows):
    ...
    if not s0:
        verdict = "WINRATIO_GAP_DATA_INVALID"
    elif flip and not same_pos:
        verdict = "WINRATIO_GAP_CONDITIONAL_FLIP"
    elif not s1:
        if (m25 > 0) != (m26 > 0) or q.get("majority_negative"):
            verdict = "WINRATIO_GAP_CONDITIONAL_FLIP"
        else:
            verdict = "WINRATIO_GAP_CONDITIONAL_NO_EDGE"
    else:
        verdict = "WINRATIO_GAP_FEATURE_CANDIDATE_A_ONLY"  # A pass boolean; final success only after C
        # per knife: A pass is gate boolean not intermediate label; use a private marker
        verdict = "A_PASS"
    # Map A_PASS to continue signal; final labels only at end. For A-stop use NO_EDGE etc.
    if verdict == "A_PASS":
        final = "A_GATE_PASS"
    else:
        final = verdict
...
def main(argv=None):
    ...
    if args.fail_if_out_exists:
        for name in ("daily_partial_rankic.csv", "window_summary.json", "verdict.json"):
            if (out_dir / name).exists():
                raise FileExistsError(out_dir / name)
    out_dir.mkdir(parents=True, exist_ok=True)

    cyq_sha = _sha256(args.cyq_file.expanduser().resolve())
    if cyq_sha.lower() != args.cyq_sha256.lower():
        raise RuntimeError(f"CYQ SHA mismatch {cyq_sha}")
    ...
    if manifest.get("sidecar_sha256", "").lower() != side_sha.lower():
        raise RuntimeError("manifest sidecar sha mismatch")

    sc = pd.read_parquet(side)
    req = [c.strip() for c in args.require_columns.split(",")]
    missing = [c for c in req if c not in sc.columns]
    if missing:
    ...
    if args.verify_residual_orthogonality:
        f = sc.dropna(subset=[args.feature, args.control_q_column])
        corr = float(np.corrcoef(f[args.feature], f[args.control_q_column])[0, 1])
        if abs(corr) > 1e-6:
            # OLS residual vs q should be ~0; allow tiny float noise
            if abs(corr) > 1e-4:
                raise RuntimeError(f"g not orthogonal to q: corr={corr}")
    ...
    for name, (start, end) in windows.items():
        ...
        merged = merged.merge(
            sc[["datetime", "instrument", args.feature, args.control_q_column]],
            on=["datetime", "instrument"],
            how="left",
        )
        n_score = int(merged["score"].notna().sum())
        complete = merged.dropna(subset=["score", "label", args.feature, args.control_q_column])
        coverage = len(complete) / n_score if n_score else 0.0
        rows = []
        deg = {}
        for day, g in complete.groupby("datetime", sort=True):
            pic, reason = _partial_day(g, args.feature, args.control_q_column, args.min_partial_n)
            if reason:
                deg[reason] = deg.get(reason, 0) + 1
                continue
            rows.append({"window": name, "datetime": _date_string(day), "n_complete": int(len(g)), "partial_rankic": pic})
        daily = pd.DataFrame(rows)
        if daily.empty:
            raise RuntimeError(f"{name}: no effective days")
        stats = _series_stats(daily["partial_rankic"])
        boot = moving_block_bootstrap_ci(daily["partial_rankic"], block_days=args.block_days, reps=args.bootstrap_reps, seed=args.seed)
        ...
        print(f"[{name}] mean={stats['mean']:.6f} ci=[{boot['ci_lower']:.6f},{boot['ci_upper']:.6f}] cov={coverage:.4f}", flush=True)

    decision = decide_verdict(summaries)
    # A-only terminal mapping per knife §4: if not S1_pass, decision.verdict already failure label
    if decision["gates"]["S1_pass"]:
        decision["verdict"] = "A_GATE_PASS"
    ...
    (out_dir / "window_summary.json").write_text(json.dumps(_json_safe(payload), ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "verdict.json").write_text(json.dumps(_json_safe(decision), ensure_ascii=False, indent=2), encoding="utf-8")
