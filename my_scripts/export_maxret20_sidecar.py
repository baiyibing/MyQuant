# -*- coding: utf-8 -*-
"""T5-MXR1: one-shot MAXRET20_ANTI_RANK sidecar on frozen handler index."""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Export MAXRET20_ANTI_RANK sidecar once")
    ...
    p.add_argument("--daily-return", default="close-to-close")
...
def _require_frozen(args) -> None:
    ...
    if args.daily_return != "close-to-close":
        raise ValueError(f"daily-return must be close-to-close, got {args.daily_return}")
    if args.aggregate != "max":
        raise ValueError(f"aggregate must be max, got {args.aggregate}")
    if int(args.multiply_before_rank) != -1:
        raise ValueError(f"multiply-before-rank must be -1 (max then * -1), got {args.multiply_before_rank}")
...
def _recompute_one(instrument: str, t, calendar, close_field: str, lookback: int, pd, np):
    ...
    ret = close / prev - 1.0  # simple close-to-close; never abs / log
    window_ret = ret.loc[window_days]
    if not np.isfinite(window_ret.to_numpy(dtype=float)).all():
        return None, "nonfinite_return"
    # max first, then multiply -1 happens only at rank time
    return float(window_ret.max()), ""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ret = close_w / prev - 1.0  # simple close-to-close; not abs, not log
    ...
    # max(r) first; multiply by -1 only immediately before rank
    maxret20_w = ret_d.rolling(args.lookback_market_days, min_periods=args.lookback_market_days).max()
    maxret20_w = maxret20_w.where(n_ok == args.lookback_market_days)
    ret_min20_w = ret_d.rolling(args.lookback_market_days, min_periods=args.lookback_market_days).min()
    ret_min20_w = ret_min20_w.where(n_ok == args.lookback_market_days)
    ret_mean20_w = ret_d.rolling(args.lookback_market_days, min_periods=args.lookback_market_days).mean()
    ret_mean20_w = ret_mean20_w.where(n_ok == args.lookback_market_days)
    close_lag20_w = close_w.shift(args.lookback_market_days)
    ...
    long = _melt(maxret20_w.loc[start:end], "maxret20")
    ...
    long = long.merge(_melt(ret_min20_w.loc[start:end], "ret_min20"), on=["datetime", "instrument"], how="left")
    long = long.merge(_melt(ret_mean20_w.loc[start:end], "ret_mean20"), on=["datetime", "instrument"], how="left")
    ...
    long["window_complete"] = (long["n_valid_window_days"] == args.lookback_market_days) & long["maxret20"].notna()
    long["missing_reason"] = [
    ...
    # rank ONLY on handler-day ∩ finite maxret20, on (-maxret20), never full-market first
    ok = out["maxret20"].notna() & np.isfinite(out["maxret20"])
    out[args.feature_name] = np.nan
    neg = -out.loc[ok, "maxret20"]
    ...
    if args.sample_recompute > 0 and len(finite):
        ...
        if n_bad:
            raise RuntimeError(f"sample maxret20 recompute failed: {n_bad}/{len(sample_qc)}")
        print(f"[qc] maxret20 recompute ok n={len(sample_qc)}", flush=True)
    ...
    if days:
        ...
        for day in day_picks:
            ...
            valid = g["maxret20"].notna() & np.isfinite(g["maxret20"])
            recomputed = (-g.loc[valid, "maxret20"]).rank(method="average", pct=True)
        ...
        print(f"[qc] rank(-maxret20) recompute ok days={len(rank_qc)}", flush=True)
    ...
    manifest = {
        ...
        "formula": "maxret20=max(close_d/close_{d-1}-1 for d in t-19..t); rank_pct(-maxret20) on handler-day ∩ valid maxret20",
        "rank": "pandas.Series.rank(method=average, pct=True) on -maxret20; max first then * -1; never max(-r)",
        ...
        "timing_ok": timing_ok,
        "direction": "higher_MAXRET20_ANTI_RANK_higher_expected_return",
        "no_sign_flip": True,
        "no_amount_volume_filter": True,
