# -*- coding: utf-8 -*-
"""同 pred 50/5，只开 ST + 上市满 60 日，买入状态关。不重训，不覆盖已有分析包。"""
...
OUT_ANALYSIS = (
    Path(__file__).resolve().parents[1]
    / "exports/analysis/8a061ea4_50n5_st_age"
)
...
def main() -> int:
    ...
    print("[st-age] ST+age ON, buy-state OFF; same pred; new analysis dir", flush=True)
    ...
    for tier, costs in COST_TIERS.items():
        print(f"[st-age] tier={tier} {costs}", flush=True)
    ...
    print(f"[st-age] json {OUT_JSON}", flush=True)
    ...
    print(f"[st-age] analysis {OUT_ANALYSIS}", flush=True)
    try:
        uninstall()
        t_rec.print_summary()
    except Exception:
